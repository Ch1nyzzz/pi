...
- Removed the `@ch1nyzzz/pi-agent-core/base` selective-provider entrypoint; use the root package with an explicit `Models` instance instead.
...
- Added `@ch1nyzzz/pi-agent-core/base` for bundlers that want to pair the agent core with selective `@ch1nyzzz/pi-ai/base` provider registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
