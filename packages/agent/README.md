...
Stateful agent with tool execution and event streaming. Built on `@ch1nyzzz/pi-ai`.

## Installation

```bash
npm install @ch1nyzzz/pi-agent-core
```

## Quick Start

```typescript
import { Agent } from "@ch1nyzzz/pi-agent-core";
...
```typescript
declare module "@ch1nyzzz/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@ch1nyzzz/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@ch1nyzzz/pi-agent-core";
