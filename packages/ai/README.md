...
```bash
npm install @ch1nyzzz/pi-ai
```

TypeBox exports are re-exported from `@ch1nyzzz/pi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@ch1nyzzz/pi-ai';
import { builtinModels } from '@ch1nyzzz/pi-ai/providers/all';
...
```typescript
import { anthropicProvider } from '@ch1nyzzz/pi-ai/providers/anthropic';
import { openaiProvider } from '@ch1nyzzz/pi-ai/providers/openai';
import { openrouterProvider } from '@ch1nyzzz/pi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@ch1nyzzz/pi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@ch1nyzzz/pi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@ch1nyzzz/pi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@ch1nyzzz/pi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@ch1nyzzz/pi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@ch1nyzzz/pi-ai';
...
```typescript
import { builtinImagesModels } from '@ch1nyzzz/pi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@ch1nyzzz/pi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@ch1nyzzz/pi-ai';
import { openAICompletionsApi } from '@ch1nyzzz/pi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@ch1nyzzz/pi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@ch1nyzzz/pi-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@ch1nyzzz/pi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@ch1nyzzz/pi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@ch1nyzzz/pi-ai';
import { anthropicProvider } from '@ch1nyzzz/pi-ai/providers/anthropic';
import { openaiProvider } from '@ch1nyzzz/pi-ai/providers/openai';
import { googleProvider } from '@ch1nyzzz/pi-ai/providers/google';
...
```typescript
import { createModels } from '@ch1nyzzz/pi-ai';
import { anthropicProvider } from '@ch1nyzzz/pi-ai/providers/anthropic';
...
- `@ch1nyzzz/pi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@ch1nyzzz/pi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@ch1nyzzz/pi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@ch1nyzzz/pi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@ch1nyzzz/pi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@ch1nyzzz/pi-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@ch1nyzzz/pi-ai/bedrock-provider';
...
```bash
npx @ch1nyzzz/pi-ai login              # interactive provider selection
npx @ch1nyzzz/pi-ai login anthropic    # login to specific provider
npx @ch1nyzzz/pi-ai list               # list available providers
```
...
The legacy flow functions remain available via the `@ch1nyzzz/pi-ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@ch1nyzzz/pi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@ch1nyzzz/pi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@ch1nyzzz/pi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@ch1nyzzz/pi-ai`.
